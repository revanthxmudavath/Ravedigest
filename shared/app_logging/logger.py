...
def setup_logging(
    ...
    include_corr_id = (
        ...
        else settings.logging.include_correlation_id
    )
...
def log_function_call(logger: logging.Logger, func_name: str, **kwargs) -> None:
    ...
    logger.debug(
        f"Calling {func_name}", extra={"function": func_name, "parameters": kwargs}
    )


def log_function_result(
    logger: logging.Logger, func_name: str, result: Any = None, **kwargs
) -> None:
