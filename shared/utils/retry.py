        ...
        def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(config.max_retries + 1):
                try:
                    ...
                    logger.warning(
                        f"Function {func.__name__} failed (attempt {attempt + 1}/{config.max_retries + 1}): {e}. Retrying in {delay:.2f}s"
                    )
        ...
        async def wrapper(*args, **kwargs) -> Any:
            ...
            for attempt in range(config.max_retries + 1):
                try:
                    ...
                    if attempt == config.max_retries:
                        logger.error(
                            f"Async function {func.__name__} failed after {config.max_retries} retries: {e}"
                        )
                        raise RetryError(
                    ...
                    delay = calculate_delay(attempt, config)
                    logger.warning(
                        f"Async function {func.__name__} failed (attempt {attempt + 1}/{config.max_retries + 1}): {e}. Retrying in {delay:.2f}s"
                    )
                    ...
                    # Non-retryable exception, re-raise immediately
                    logger.error(
                        f"Async function {func.__name__} failed with non-retryable exception: {e}"
                    )
                    raise
