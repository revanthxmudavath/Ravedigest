...
class HealthChecker:
    ...
    def get_health_summary(self) -> Dict[str, Any]:
        ...
        healthy_checks = sum(1 for check in health_data["checks"] if check["status"] == "healthy")
...
def create_collector_health_checker() -> HealthChecker:
    ...
    for i, feed_url in enumerate(settings.service.rss_feeds[:3]):  # Check first 3 feeds
        checker.add_check(lambda url=feed_url, idx=i: checker.check_http_endpoint(url, f"rss_feed_{idx}"))
    return checker
