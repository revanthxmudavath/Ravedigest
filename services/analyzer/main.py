...
ARTICLE_PROCESSED = Counter(
    "analyzer_article_processed_total", "Total number of articles processed"
)

app = FastAPI(
    title="Rave Digest Analyzer",
    description="Analyzes articles for developer focus and summarizes them.",
)
...
def get_analyzer_status():
    ...
    try:
        ...
        if not group_info:
            raise HTTPException(
                status_code=404, detail=f"Consumer group {group_name} not found."
            )
        ...
        if "no such key" in str(e).lower():
            return {"is_idle": True, "status": "Stream not found, assuming idle."}
                ...
                for msg_id, payload in messages:
                    ...
                    try:
                        await handle_message(
                            payload, msg_id, redis_client, stream, group
                        )
                    except Exception as e:
                        logger.error(f"Error processing message {msg_id}: {e}")
