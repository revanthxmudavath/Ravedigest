    ...
    while True:
        try:
            entries = redis_client.xreadgroup(
                group, consumer, {stream: ">"}, block=5000
            )
            if entries:
                for _, msgs in entries:
                    for msg_id, payload in msgs:
                        try:
                            ...
                            try:
                                logger.info(
                                    "📩 Received enriched article %s; composing digest",
                                    msg_id,
                                )
                                ...
                                if digest_result is not None:
                                    ...
                                    logger.info(
                                        "⏭️ No digest generated for %s (no articles available)",
                                        msg_id,
                                    )
                                    redis_client.xack(
                                        stream, group, msg_id
                                    )  # Still acknowledge as this isn't an error
